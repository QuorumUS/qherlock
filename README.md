...
Each anomaly is diagnosed with Claude and closed through a guarded fix chain — trigger Quorum's own re-ingestion first, fall back to a pre-approved ORM fix template, roll back and alert if verification fails — with every action logged to the `#sherlock-bot` Slack channel.

## Trust model

Sherlock earns autonomy in stages — same code, one switch:

1. **Shadow mode** *(start here)* — detect, diagnose, and report to `#sherlock-bot`. No writes, ever.
