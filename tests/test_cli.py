...
from querlock.agent.patrol import PatrolFatalError
from querlock.cli import app
...
def test_sync_command_runs_with_fake_env(tmp_path, monkeypatch):
    ...
    # No network in tests: point sync at a fake client via QUERLOCK_TEST_MODE guard.
    monkeypatch.setenv("QUERLOCK_TEST_MODE", "1")
...
def test_sync_scope_list_test_mode(fake_env):
    result = runner.invoke(app, ["sync", "--scope", "CA,TX"],
                            env={"QUERLOCK_TEST_MODE": "1", **fake_env})
...
def test_patrol_fatal_exits_2(fake_env, monkeypatch):
    ...
    monkeypatch.setattr("querlock.cli.run_patrol", boom)
