...
def test_settings_defaults(monkeypatch, tmp_path):
    ...
    assert s.querlock_model == "claude-sonnet-5"
    assert s.querlock_max_turns == 100
    s.ensure_dirs()
...
def test_m1_defaults(monkeypatch):
    ...
    monkeypatch.delenv("QUERLOCK_FRESHNESS_SLA_HOURS", raising=False)
    ...
    assert s.querlock_freshness_sla_hours == 72
...
def test_m1_env_overrides(monkeypatch):
    ...
    monkeypatch.setenv("QUERLOCK_FRESHNESS_SLA_HOURS", "24")
    s = Settings(legiscan_api_key="k", _env_file=None)
    assert s.slack_bot_token == "xoxb-env-token"
    assert s.slack_channel_id == "CENV"
    assert s.querlock_freshness_sla_hours == 24
